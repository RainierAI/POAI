...
final class AppState: ObservableObject {
    ...
    func loadAppConfigAndModels(channel: FlutterMethodChannel) {
        ...
        loadExampleModelsConfig(exampleModels: appConfig.exampleModels, channel: channel)
        
        for modelstate in models {
            
//            if (modelstate.modelConfig.localID == "Llama-2-7b-chat-hf-q3f16_1") {
//                print("Llama Model start chat")
//                modelstate.startChat(chatState: chatState, channel: channel)
//            }
            print("Model local ids: \(modelstate.modelConfig.localID)")
            if (modelstate.modelConfig.localID == "RedPajama-INCITE-Chat-3B-v1-q4f16_1") {
                print("Redpajama Model start chat")
                modelstate.startChat(chatState: chatState, channel: channel)
            }
        }
...
private extension AppState {
    ...
    func loadModelsConfig(modelList: [AppConfig.ModelRecord]) {
        print("Load models config start")
        ...
        print("Load models config done")
    }

    func loadExampleModelsConfig(exampleModels: [AppConfig.ModelRecord], channel: FlutterMethodChannel) {
        print("Load example models config start")
        self.exampleModels = exampleModels.map{
            ExampleModelConfig(modelURL: $0.modelURL, localID: $0.localID)
        }
        print("Load example models config done")
    ...
    func loadPrebuiltModels() {
        // models in dist
        do {
            print("Load prebuilt Models start")
            ...
            for modelDir in modelDirs {
                ...
                if fileManager.fileExists(atPath: modelConfigURL.path()) {
                    if let modelConfig = loadModelConfig(modelConfigURL: modelConfigURL) {
                        ...
                        print("Model add succeed")
                    }
                }
            }
            print("Load prebuilt Models done")
        } catch {
